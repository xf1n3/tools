...
function boneWorldBounds(bone) {
  const min = [
    bone.center[0] - bone.size[0] / 2,
    bone.center[1] - bone.size[1] / 2,
    bone.center[2] - bone.size[2] / 2,
  ];
  const max = [
    bone.center[0] + bone.size[0] / 2,
    bone.center[1] + bone.size[1] / 2,
...
function updateQuickReadout() {
  ...
  const vals = mode === 'translate' ? bone.center : bone.size;
...
function recompute() {
  ...
  const boneDefs = state.bones
    .filter(b => b.enabled)
    .map(b => {
      const { min, max } = boneWorldBounds(b);
      return { name: b.name, enabled: true, min, max };
    });
