...
/**
 * Дублирует кость целиком: создаёт новую кость с тем же числом боксов,
 * теми же размерами/поворотами, но чуть смещённую в сторону, чтобы новый
 * куб не лежал ровно поверх исходного и его сразу было видно и можно было
 * двигать отдельно.
 */
function duplicateBone(boneIndex) {
  const source = state.bones[boneIndex];
  const firstBox = source.boxes[0];
  const offsetAmount = Math.max(firstBox.size[0], firstBox.size[1], firstBox.size[2]) * 1.2 || 0.3;

  const newBoneIndex = addBone(
    [firstBox.center[0] + offsetAmount, firstBox.center[1], firstBox.center[2]],
    firstBox.size.slice(),
    `${source.name}_copy`
  );
  const newBone = state.bones[newBoneIndex];
  // Копируем поворот первого бокса (addBone уже создал box 0 с этим center/size)
  newBone.boxes[0].rotation = firstBox.rotation.slice();
  applyBoxStateToMesh(newBoneIndex, 0);

  // Остальные боксы кости (если это была объединённая кость из нескольких кубов)
  for (let i = 1; i < source.boxes.length; i++) {
    const b = source.boxes[i];
    const newCenter = [b.center[0] + offsetAmount, b.center[1], b.center[2]];
    const newBoxIdx = addBoxToBone(newBoneIndex, newCenter, b.size.slice());
    newBone.boxes[newBoxIdx].rotation = b.rotation.slice();
    applyBoxStateToMesh(newBoneIndex, newBoxIdx);
  }
  ...
  state.bones.forEach((bone, i) => {
    ...
    card.innerHTML = `
      ...
      <div class="bone-actions-row">
        <button class="bone-mini-btn" data-idx="${i}" data-role="add-box">+ куб</button>
        <button class="bone-mini-btn" data-idx="${i}" data-role="duplicate-bone">Дублировать</button>
        <button class="bone-mini-btn danger" data-idx="${i}" data-role="delete-bone">Удалить</button>
      </div>
...
function wireBonesSheetEvents(bonesListEl) {
  ...
  bonesListEl.querySelectorAll('[data-role="duplicate-bone"]').forEach(btn => {
    btn.addEventListener('click', () => {
      const idx = Number(btn.dataset.idx);
      const newIdx = duplicateBone(idx);
      selectBox(newIdx, 0);
      renderBonesSheetBody();
      scheduleRecompute();
      setStatus('Кость продублирована');
    });
  });
