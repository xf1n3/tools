...
export function parseBedrockGeometry(json) {
  ...
  for (const bone of bones) {
    ...
    // Вариант 1: polyMesh (это то, что обычно получается при конвертации .obj -> bedrock)
    if (bone.polyMesh) {
      const pm = bone.polyMesh;
      const g = polyMeshToGroup(pm, boneName, pivot);
      if (g) groups.push(g);
      continue;
    }
  ...
  if (groups.length === 0) {
    throw new Error('Не удалось найти геометрию (ни polyMesh, ни cubes) ни в одной кости');
  }
